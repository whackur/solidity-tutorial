...
A single `SpotPriceBasicLab` is deployed. Each user calls `createInstance()` to get their own `MockPool` seeded with `1000 A / 1000 B`, so the starting spot price is `1.0` (1e18). The pool is **tokenless** — there are no ERC-20s and no approvals. Swaps just bump the reserves according to xy=k.
...
Make `SpotPriceBasicLab.isSolved(yourAddress)` return `true`. That requires:
...
1. `lab.createInstance()` — deploys your personal `MockPool(1000e18, 1000e18, you)`.
